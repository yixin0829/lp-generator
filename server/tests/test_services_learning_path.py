...
class TestCheckModeration:
    def test_passes_when_not_flagged(self, service: LearningPathService, mock_client):
        mock_client.moderations.create.return_value = MagicMock(results=[MagicMock(flagged=False)])
        service.check_moderation("React")
        mock_client.moderations.create.assert_called_once_with(input="React")

    def test_raises_when_flagged(self, service: LearningPathService, mock_client):
        ...
        with pytest.raises(ContentModerationError, match="content policy"):
            service.check_moderation("badword")


class TestGenerateLearningPath:
    def test_success_returns_expected_structure(self, service: LearningPathService, mock_client):
        ...
        result = service.generate_learning_path("react")
    ...
    def test_malformed_json_raises(self, service: LearningPathService, mock_client):
        mock_client.moderations.create.return_value = MagicMock(results=[MagicMock(flagged=False)])
        mock_client.responses.create.return_value = MagicMock(
        ...
        with pytest.raises(MalformedResponseError, match="parsing"):
            service.generate_learning_path("react")
